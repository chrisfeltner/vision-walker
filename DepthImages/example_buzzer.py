...
class haptic():
    ...
    def bRead(self, register):
        return self.bus.read_byte_data(DRV2605_ADDR, register)

    def bWrite(self, register, value):
        return self.bus.write_byte_data(DRV2605_ADDR, register, value)

    def readDeviceID(self):
        status = self.bRead(self.DRV2605_REG_STATUS)
        return self.DRV2605_DEVICE_ID[status >> 5]

    def readStatus(self):
        status = bRead(self.DRV2605_REG_STATUS)
        if status & 0xF:
            return "abnormal"
        else:
            return "normal"

    def selectLibrary(self,libraryValue):
        ...
        bWrite(self.DRV2605_REG_LIBRARY, libraryValue)

    def __init__(self):
        print("DRV2605 device ID: " + self.readDeviceID())
        print("DRV2605 status: " + readStatus())

        # Clear the STANDBY bit to exit the standby state (and go to the ready state)
        bWrite(self.DRV2605_REG_MODE, 0x00)
        ...
        bWrite(self.DRV2605_REG_CONTROL3, bRead(self.DRV2605_REG_CONTROL3) | 0x20)

    def playEffect(self, effect):
        # See Appendix A of http://www.ti.com/lit/ug/slau543/slau543.pdf
        bWrite(self.DRV2605_REG_WAVESEQ1, effect)
        bWrite(self.DRV2605_REG_WAVESEQ2, 0)
        bWrite(self.DRV2605_REG_GO, 1)
